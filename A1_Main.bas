...
Sub PrepareOverviewSheet(FilesListSring As String)
    ...
    With Worksheets("RAPPORT")
        ...
        .Range(Chr(Asc("K") + HOffset) & VOffset).value = "Champs inconnus"
            .Range(Chr(Asc("K") + HOffset) & VOffset).AddComment ("Les attributs reportés sont inconnus de l'application." & Chr(10) & "Enregistrez-les dans la table [attributes] de" & Chr(10) & "la feuille de calcul cachée [INTERNALS]" & Chr(10) & "et assignez leur le n° de colonne DBB_col approprié.")
